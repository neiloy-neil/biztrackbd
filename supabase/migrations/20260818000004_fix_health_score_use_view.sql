-- get_business_health_score was reading parties.current_due (stale cache)
-- for its receivables scoring. Same root cause as get_dashboard_summary:
-- POS payments via account_transactions don't fire the trigger that
-- updates parties.current_due, so the cached column diverges over time.
-- Also adds current_due > 0 filter so negative stale rows don't lower the total.
CREATE OR REPLACE FUNCTION public.get_business_health_score(p_business_id uuid, p_start_date date, p_end_date date)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
